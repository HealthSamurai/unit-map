(ns unit-map.ops-test
  (:require [unit-map.ops :as sut]
            ...
            [unit-map.type.chrono.util.misc :as misc]
            [clojure.test :as t]))


#_"TODO: use sut/eq? instead of t/is"
...
(t/deftest type-exception-test
  ...
  (try (sut/definition {:hour 1})
       ...
       (finally (t/is (= (-> (sut/no-default-type-exception {:hour 1})
                         ...
                         (select-keys exception [:class :data])))))
...
(t/deftest ops-test
  ...
  (t/testing "process-sequence"
    ...
    (t/is (= [{:start 1, :step 1, :end misc/days-in-month}]
             (sut/process-sequence days)))

    (def f (fn [{:keys [bar]}] (if (odd? bar) 9 11)))

    (t/is (= [{:start 1, :step 2, :end f} 13 15]
             #unit-map/sequence[1 3 .. :TODO-REMOVE f 13 15])))
